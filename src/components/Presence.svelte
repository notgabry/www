...
<div class="group h-20 max-w-md my-2 mx-auto block px-2 py-[0.45rem] border-2 border-black bg-p-card-4 relative shadow-neobrutalist-low">
    {#if isLoading}
    ...
    <div class="h-16 flex">
        <div class="max-w-[4rem] flex flex-shrink-0 h-full">
            <img on:load={() => (isLoading = false)} src={data?.image} class="max-w-full h-full aspect-square flex-grow-0" draggable="false" alt="Discord Presence" />
        </div>
